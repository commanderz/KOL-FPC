...
begin
  ...
  if FE then
  begin
    DeleteFile( PKOLChar( Res + '_tmp' ) );
    CopyFile( PKOLChar(Res), PKOLChar( (Res+'_tmp') ), False );
  end;

  hFR := CreateFile( PKOLChar(Res), GENERIC_READ or GENERIC_WRITE, FILE_SHARE_READ,
  ...
  hFtm := CreateFile( PKOLChar(Bmp), GENERIC_READ, FILE_SHARE_READ, nil,
  ...
  if FE then begin
      hFtm := CreateFile( PKOLChar( (Res+'_tmp') ), GENERIC_READ,
...
begin
  ...
  try
    ...
    begin
      ...
      if FileExists( Path + '.$$$' ) then
        DeleteFile( Path + '.$$$' );
      MoveFile( PKOLChar( Path ), PKOLChar( Path + '.$$$' ) );
    end;
    MS.Position := 0;
    MS.SaveToFile( Path );
    //Result := True;
