...
********************************************************
* VERSION 3.20         modified by Nikolas for FPC
********************************************************
}                     
...
{$IFDEF VER150}
  // Use KOLCTRLWRAPPER on Delphi7 by default
...
uses Windows, Messages, KOL, KOLadd, Classes, Forms, Controls, Dialogs, extctrls,
...
{$else}
     ComponentEditors, PropEdits, Variants, LResources,
     LazIdeIntf, SrcEditorIntf, ProjectIntf, CodeToolManager,
     DefineTemplates,
{$endif FPC}
     TypInfo,
...
{$ENDIF}
  ...
  published
    ...
    // имя-проекта.res).
    property dprResource: Boolean read fDprResource write fDprResource default False;
    ...
    property protectFiles: Boolean read fProtect write fProtect default True;

    property showReport: Boolean read GetShowReport write fShowReport default False;
    ...
    property isKOLProject: Boolean read GetIsKOLProject write SetIsKOLProject default True;

    property autoBuild: Boolean read fAutoBuild write SetAutoBuild default True;
    property autoBuildDelay: Integer read FAutoBuildDelay write SetAutoBuildDelay default 500;
...
{$ifdef FPC}
  TKOLObjectCompEditor = class( TDefaultComponentEditor )
  public
    procedure Edit; override;
  end;
{$else Delphi}
...
{$ifdef FPC}
    procedure EditProperty(const PropertyEditor: TPropertyEditor;
              var Continue: Boolean); override;
{$else}
...
{YS}//--------------------------------------------------------------
  ...
  public
    procedure GetChildren(Proc: TGetChildProc {$IFDEF _D3orHigher} ; Root: TComponent {$ENDIF} );
...
function IsHandlersEqual(const Handler1, Handler2: TMethod): boolean;
function IsHandlerEmpty(const Handler: TMethod): boolean;
function GetEventHandlerName(ParentContainer, C: TComponent; const EventName: string): string;
...
uses {$ifdef win32} ShellAPI {$IFNDEF _D2}, shlobj, ActiveX {$ENDIF},{$endif win32}
     mckCtrls, mckObjs;
...
{$endif FPC}
  begin
{$ifdef FPC}
    // Define LAZIDE_MCK specially for Lazarus IDE.
    dt:=TDefineTemplate.Create('MCK','MCK defines','','',da_Block);
    dt.Flags:=[dtfAutoGenerated];
    dt.AddChild(TDefineTemplate.Create('Define LAZIDE_MCK', '','LAZIDE_MCK','',da_DefineRecurse));
    CodeToolBoss.DefineTree.Add(dt);
...
function IsHandlersEqual(const Handler1, Handler2: TMethod): boolean;
begin
  Result:=(Handler1.Data = Handler2.Data) and (Handler1.Code = Handler2.Code);
end;

function IsHandlerEmpty(const Handler: TMethod): boolean;
begin
  Result:=(Handler.Data = nil) and (Handler.Code = nil);
end;

{$ifdef FPC}
function LazGetEventHandlerName(C: TComponent; const EventName: string): string;
var
  pi: PPropInfo;
  PropEditor: TMethodPropertyEditor;
  i: integer;
begin
  Result:='';
  pi:=GetPropInfo(C, EventName);
  if (pi <> nil) and (pi.PropType.Kind = tkMethod) then begin
    PropEditor := TMethodPropertyEditor.Create(GlobalDesignHook,1);
    PropEditor.SetPropEntry(0, C, pi);
    PropEditor.Initialize;
    if PropEditor.ValueAvailable then
      Result:=PropEditor.GetValue;
    PropEditor.Free;
    i:=Pos('.', Result);
    if i > 0 then
      Delete(Result, 1, i);
...
begin
  Result:='';
  m:=GetMethodProp(C, EventName);
  if (m.Code <> nil) and (ParentContainer <> nil) then
    Result:=ParentContainer.MethodName(m.Code)
{$ifdef FPC}
  else
    Result:=LazGetEventHandlerName(C, EventName);
{$endif FPC};
end;
...
begin
  ...
  Result := C is controls.TControl;
  if Result then
  if (C is TKOLApplet) or (C is TKOLCustomControl) {$ifndef FPC} or (C is TOleControl) {$endif}
  then result:=false
  else begin
    result:=false;
    if c.tag<>cKolTag
...
{$ifdef FPC}
  if LazarusIDE <> nil then
    Result := ExtractFilePath(LazarusIDE.ActiveProject.MainFile.Filename);
{$else}
  begin
...
{$ifdef FPC}
  if LazarusIDE <> nil then
    Result := ExtractFileNameWOExt(LazarusIDE.ActiveProject.MainFile.Filename);
{$else}
    if ToolServices <> nil then
...
begin
  ...
  si:=GetSourceEditorInterface(Path);
  if si <> nil then
    SL.Text:=si.SourceText
  else
    if FileExists( Path ) then
      SL.LoadFromFile( Path );
end;
...
begin
  ...
  if Result then begin
    ...
    si.SelEnd:=MaxInt;
    s:=SL.Text;
    if Copy(s, 1, Length(Utf8BOM)) = Utf8BOM then
      Delete(s, 1, Length(Utf8BOM));
    si.Selection:=s;
    si.SelStart:=0;
    si.SelEnd:=0;
    si.UpdateCodeBuffer;
...
begin
  ...
  if si <> nil then begin
    Rpt( 'Update Unit: ' + Path, WHITE ); //Rpt_Stack;
    LazarusIDE.DoSaveEditorFile(si, []);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      s := Old[ I ];
      if StrIsStartingFrom( PKOLChar( AnsiString(s) ), ' PROC(2) //--by PCompiler:line#' ) then // TODO: dangerous
        Old[ I ] := ' PROC(2)'
      else
  ...
  begin
    ...
    begin
      //Rpt( 'updated (in IDE Editor): ' + Path );
      if FileExists( Path ) then
        SetFileAttributes( PKOLChar( Path ), FILE_ATTRIBUTE_NORMAL );
      Updated := TRUE;
      Exit;
    end;
    ...
    if FileExists( S1 ) then
      DeleteFile( S1 );
    SetFileAttributes( PKOLChar( Path ), FILE_ATTRIBUTE_NORMAL );
    MoveFile( PKOLChar( Path ), PKOLChar( S1 ) );
    if KOLProject <> nil then
    begin
      S1 := KOLProject.OutdcuPath + ExtractFileName( Path );
      if LowerCase( Copy( S1, Length( S1 ) - 3, 4 ) ) = '.inc' then
...
procedure TKOLCtrlWrapper.RecreateKOLControl;
begin
  if FKOLCtrl = nil then exit;
{$IFNDEF NOT_USE_KOLCTRLWRAPPER}
  if HandleAllocated then
...
begin
  ...
  begin
    ...
    begin
      ...
      END;
      DrawText( Canvas.Handle, PKOLChar( Txt ), Length( Txt ), R, Flags ); // TODO: dangerous
...
{$ifdef FPC}
      ...
      Handler:=LazGetEventHandlerName(Self, EventNames[I]);
      if Handler <> '' then
        SL.Add( '      ' + AName + '.' + AnsiString(EventNames[ I ]) + ' := Result.' + Handler + ';' );
{$endif FPC};
...
begin
  ...
  try
    ...
    if (fSourcePath = '') or not DirectoryExists( fSourcePath ) or
       ({$ifdef FPC}LazarusIDE{$else}ToolServices{$endif} = nil) or not(Self is TKOLForm) then
    begin
...
{$ifdef FPC}
        ...
        n:=EventNames[I];
        if n = 'OnFormClick' then
          n:='OnClick';
        Handler:=LazGetEventHandlerName(Self, n);
      if Handler <> '' then
        SL.Add( '      ' + AName + '.' + AnsiString(EventNames[ I ]) + ' := Result.' + Handler + ';' );
      end
{$endif FPC};
...
begin
  ...
  FIsDestroying := TRUE;
  try
  FRealignTimer.Free;//from by Nikolas
  FChangeTimer.Free;// from by Nikolas
  if bounds <> nil then
...
procedure TKOLForm.WriteState(Writer: TWriter);
{$ifdef FPC}
var
  lp: TLazProjectFile;
{$endif FPC}
begin
{$ifdef FPC}
  lp:=LazarusIDE.GetProjectFileWithRootComponent(Owner);
  if (lp <> nil) and (KOLProject <> nil) and (KOLProject.FprojectDest = 'empty_temp_project') then begin
    KOLProject.sourcePath:=ExtractFilePath(lp.Filename);
    KOLProject.FoutdcuPath:=KOLProject.sourcePath;
    UnitSourcePath:=KOLProject.sourcePath;
  end;
{$endif FPC}
  inherited;
{$ifdef FPC}
  if (lp <> nil) and not FileExists(ChangeFileExt(lp.Filename, '_1.inc')) then
    Change(Self);
{$endif FPC}
end;
  ...
  //--------------------------
    ...
    TRY
      ...
      try
        ...
        SL.Add( {$ifdef FPC} Utf8BOM + {$endif} Signature );
        if Generate_Pcode then
        begin
        ...
        begin
          ...
          // Resolve links for actions
          if TComponent( oc[ I ] ) is TKOLAction then
            TKOLAction( oc[ I ] ).ResolveLinks;
        end;
...
{$endif FPC}
    ...
    {$ifndef FPC} or (TComponent( oc[ I ] ) is TOlecontrol) {$endif} ) then
...
{$ifdef FPC}
    LazFormDefCommentFound: boolean;
{$endif FPC}
    S: KOLString;
...
{$ifdef FPC}
        Source[ I + 1 ] :=
          '  {$IFDEF KOLCLASSES} T' + FormName +
          ' = class; P' + FormName + ' = T' + FormName + ';' +
          ' {$ELSE OBJECTS}' +
          ' P' + FormName + ' = ^T' + FormName + ';' +
          ' {$ENDIF CLASSES/OBJECTS}';
        if RemoveSpaces(Source[I + 2]) = RemoveSpaces('{$IFNDEF LAZIDE_MCK}') then
          Source.Delete(I + 2);
        Source[ I + 2 ] :=
          '  T' + FormName + ' = {$IFDEF LAZIDE_MCK}class(TForm{$ELSE}{$IFDEF KOLCLASSES}class{$ELSE}object{$ENDIF}(TObj{$ENDIF})';
        if Pos('{$ELSE}', Source[ I + 3 ]) <> 0 then
          Source.Delete(I + 3);
{$else Delphi}
        Source[ I + 1 ] :=
...
{$ifdef FPC}
      SL.Add( DefString );
      SL.Add('{$ifdef FPC} {$mode delphi} {$codepage utf8} {$endif}');
{$endif FPC}
      for I := 0 to Source.Count - 1 do
      begin
{$ifdef FPC}
        if (pos( '.lrs}', LowerCase( Source[ I ] ) ) > 0) and (pos( '{$i ', LowerCase( Source[ I ] ) ) > 0) then begin
          Source[ I ] := '{$IFNDEF KOL_MCK} {$I '+AnsiLowerCase(formUnit)+'.lrs} {$ENDIF}';
          break;
        end;
        if pos( '{$r *.lfm}', LowerCase( Source[ I ] ) ) > 0 then begin
          Source[ I ] := '{$IFNDEF KOL_MCK} {$R *.lfm} {$ENDIF}';
          break;
        end;
{$endif FPC}
      ...
      begin
        Inc( I );
{$ifdef FPC}
        if (CompareText(Source[I], '{$mode objfpc}{$H+}') = 0) or (CompareText(Source[I], '{$mode delphi}') = 0) then begin
          if (I > 1) and (Trim(Source[I-1]) = '') and
             (I < Source.Count - 1) and (Trim(Source[I+1]) = '')
          then
            Inc(I);
          continue;
        end;
{$endif FPC}
        ...
        begin
          UsesFound := True;
{$ifdef FPC}
          SL.Add( 'uses Windows, Messages, KOL' + AdditionalUnits + ' {place your units here->}');
          SL.Add( '{$IFDEF LAZIDE_MCK}, Forms, mirror, Classes, Controls, mckCtrls, ' +
                  'mckObjs, Graphics;' );
          SL.Add( '{$ELSE} ; {$ENDIF}' );
{$else Delphi}
...
{$ifdef FPC}
        if not LazFormDefCommentFound and
           (pos( RemoveSpaces(LowerCase( '{T' + FormName + '}' )),
                RemoveSpaces(LowerCase( Source[ I ] )) ) > 0) then
        begin
          LazFormDefCommentFound := True;
          SL.Add( Source[ I ] );
          Continue;
        end;
{$endif FPC}
...
{$endif FPC}
{$ifdef FPC}
          if not(LazFormDefCommentFound) then
          begin
            SL.Add( '  { T' + FormName + ' }' );
            LazFormDefCommentFound := True;
          end;
{$endif FPC}
          ...
          ' {$ENDIF CLASSES/OBJECTS}' );
{$ifdef FPC}
          SL.Add( '  T' + FormName + ' = {$IFDEF KOLCLASSES}class{$ELSE}object{$ENDIF}({$IFDEF LAZIDE_MCK}TForm{$ELSE}TObj{$ENDIF})' );
...
begin
  ...
  if lp <> nil then begin
    ...
    S := ExtractFileName( S );
    if (S2 <> '') and (AnsiCompareText(S2, IncludeTrailingPathDelimiter(ProjectSourcePath)) <> 0) then
    begin
...
begin
  ...
  begin
    ...
    if S[ Length( S ) ] <> '\' then
      S := S + '\';
    S1 := S;
    S := S + Get_ProjectName + ProjectExt;
...
begin
  ...
  begin
    C := Owner.Components[ I ];
    if C = Self then Continue;
    if (C is controls.TControl)and(not((C is TKOLApplet) or (C is TKOLCustomControl) {$ifndef FPC} or (C is TOleControl) {$endif} ))and(c.tag=cKolTag)
    then begin
...
begin
  ...
  try
{$ifdef FPC}
  if (KOLProject <> nil) and (KOLProject.FprojectDest = 'empty_temp_project') then
    if not KOLProject.isKOLProject then begin
      FChangeTimer.Enabled := True;
      LogOK;
      exit;
    end
    else
      KOLProject.FprojectDest:='';
{$endif FPC}
  FChangeTimer.Enabled := FALSE;
  DoChangeNow;
  FChangeTimer.Enabled := FALSE;
...
begin
  ...
  try
    ...
    begin
      ...
      if {$ifdef FPC}LazarusIDE{$else}ToolServices{$endif}=nil then
      begin
...
begin
  ...
  Result := False;
  if FLocked then //or (projectDest = 'empty_temp_project') then
  begin
  ...
  S := ExtractFilePath( Path ) + ProjectName + ProjectExt;
  LoadSource( Source, S );
  if Source.Count = 0 then
  begin
    S := ExtractFilePath( Path ) + ExtractFileNameWOExt( Path ) + ProjectExt;
    LoadSource( Source, S );
  end;
  IsDLL := FALSE;
  for I := 0 to Source.Count-1 do
...
{$ifdef FPC}
  SL.Add('{$ifdef FPC} {$mode delphi} {$endif}');
{$endif FPC}
  S := '';
  I := -1;
  Kol_added := FALSE;
...
{$ifdef FPC}
    if S = '{$ifdef FPC} {$mode delphi} {$endif}' then continue;
    if CompareText(S, '{$mode objfpc}{$H+}') = 0 then begin
      if (I > 1) and (Trim(Source[I-1]) = '') and
         (I < Source.Count - 1) and (Trim(Source[I+1]) = '')
      then
        Inc(I);
...
{$ifdef FPC}
      J := pos( ', MirrorKOLPackage', S );
      if J <> 0 then
        System.Delete(S, J, 18)
      else begin
        J := pos( ', LResources', S );
        if J <> 0 then
          System.Delete(S, J, 12);
      end;
      if (Trim(S) = 'Forms') and (I < Source.Count - 1) then begin
        J:=Pos(',', Source[I+1]);
        if J <> 0 then begin
          Source[I+1]:=' ' + Copy(Source[I+1], J+1, MaxInt);
          continue;
        end;
      end;
      if (Trim(S) = '{$IFDEF UNIX}{$IFDEF UseCThreads}') and (I < Source.Count - 2) and
         (Trim(Source[I+1]) = 'cthreads,') and (Trim(Source[I+2]) = '{$ENDIF}{$ENDIF}')
      then begin
        Inc(I, 2);
        continue;
      end;
...
{$ifdef FPC}
    J := pos( AnsiLowerCase('{$r ' + projectName + '.rc}'), AnsiLowerCase( S ) );
    if J <> 0 then
      S:=StringReplace(S, '{$r ' + projectName + '.rc}', '{$R ' + projectDest + '.rc}', [rfIgnoreCase]);
{$endif FPC}
  ...
  SL.Add( '' );
{$ifdef FPC}
  SL.Add( '{$IFNDEF LAZIDE_MCK} {$I ' + ProjectDest + '_0.inc} {$ELSE}' );
{$else}
...
{$ifdef FPC}
    if UpperCase( S ) = UpperCase( '{$IFNDEF LAZIDE_MCK} {$I ' + ProjectDest + '_0.INC} {$ELSE}' ) then
      continue;
...
{$endif FPC}
  ...
  SaveStrings( SL, Path + ProjectExt, Updated );
  ...
  begin
    ...
    begin
      ...
      begin
          ...
          if (ProjectName <> '') and FileExists( SourcePath + ProjectName + ProjectExt ) then
          begin
            //ShowMessage( 'find if project Is KOL in ' + SourcePath + ProjectName + ProjectExt );
            SL := TStringList.Create;
            try
              LoadSource( SL, SourcePath + ProjectName + ProjectExt );
              for I := 0 to SL.Count - 1 do
...
begin
  ...
  TRY
    ...
    if (Result <> '') and DirectoryExists( Result ) {and (FprojectDest <> '') and
       FileExists( Result + FprojectDest + ProjectExt )} then
    begin
      LogOK; Exit;
    end;
    ...
    TRY
      try
        ...
        if not DirectoryExists( Result ) or
           not FileExists( Result + fprojectDest + ProjectExt ) or
...
{$ifdef FPC}
         if FProjectDest = 'empty_temp_project' then begin
            fGettingSourcePath := False;
            LogOK;
            Exit;
         end;
{$endif FPC}
          ...
          BI.ulFlags := $0001; //BIF_RETURNONLYFSDIRS;
...
begin
  ...
  if not FLocked then
  begin
  S := SourcePath + ProjectName + '.lpi';
    S1 := SourcePath + ProjectDest + '.lpi';
    if FileExists(S) and not FileExists(S1) then begin
      Source := TStringList.Create;
      Source.LoadFromFile(S);
      Updated:=False;
      for I:=0 to Source.Count - 1 do begin
        S:=Source[I];
        if (Pos('<Filename Value="'+ProjectName+'.lpr"/>', S) <> 0) or
           (Pos('<UnitName Value="'+ProjectName+'"/>', S) <> 0)
        then begin
          Source[I]:=StringReplace(S, '"'+ProjectName, '"'+ExtractFileNameWOext(S1), []);
          Updated:=True;
        end
        else
        if Pos(UpperCase('<PackageName Value="LCL"/>'), UpperCase(S)) <> 0 then begin
          Source[I]:=StringReplace(S, '"LCL"', '"MirrorKOLPackage"', []);
          Updated:=True;
        end;
      end;
      if Updated then
        Source.SaveToFile(S1);
      Source.Free;
    end;
  end;
  LogOK;
...
begin
  ...
  begin
   Handler := '';
    if EventHandlers[ I ] <> nil then
      Handler:=ParentForm.MethodName( EventHandlers[ I ] )
{$ifdef FPC}
    else
      Handler:=LazGetEventHandlerName(Self, EventNames[I]);;
{$endif FPC};
...
begin
  ...
  if Component is TKOLObj then
    BestEditEvent := (Component as TKOLObj).BestEventName
  else
  if Component is TKOLApplet then
    BestEditEvent := (Component as TKOLApplet).BestEventName
  else
  if Component is TKOLCustomControl then
    BestEditEvent := (Component as TKOLCustomControl).BestEventName;
  inherited;
end;
...
//*///////////////////////////////////////////////////////////////////////////////////////////////
    if FContinue then EditProperty(PropertyEditor, FContinue{$ifndef FPC}{$IFNDEF _D6orHigher}, FreeEditor{$ENDIF}{$endif}); //
//*///////////////////////////////////////////////////////////////////////////////////////////////
  finally
//*///////////////////////////////////////////////
{$IFNDEF _D6orHigher}                           //
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
{$ifdef FPC}
procedure TKOLObjectCompEditor.EditProperty(const PropertyEditor: TPropertyEditor;
...
begin
  ...
  for I := 0 to High( Cursors ) do
    if Trim( GetValue ) = Trim( Cursors[ I ] ) then
    begin
      Found := TRUE;
      break;
    end;
...
{$ifdef FPC}
      GlobalDesignHook.RenameMethod(FOnMenuMethodName, NewMethodName);
...
begin
  ...
  if Count > 0 then Exit;
{$ifdef FPC}
  CreateComponentEvent(Self, 'OnMenu');
  GlobalDesignHook.ShowMethod(FOnMenuMethodName);
  Change;
{$else Delphi}
  F := MenuComponent.ParentForm;
  if F = nil then Exit;
...
var Id: Integer;
    M: HMenu;
    MII: MenuItemInfo;//TMenuItemInfo;
    KMI: TKOLMenuItem;
    C: TControl;
    F: TForm;
    I: Integer;
...
var F: TForm;
    ...
    var MII: MenuItemInfo;//TMenuItemInfo;
        S: String;
        J: Integer;
    begin
      asm
...
begin
  ...
  TRY
    ...
    if oldM <> 0 then
      DestroyMenu( oldM );
    oldWndProc := pointer(GetWindowLong( F.Handle, GWL_WNDPROC ));
...
begin
  ...
  CurWndProc := pointer(GetWindowLong( Wnd, GWL_WNDPROC ));
...
const
  BrushStyles: array[ TBrushStyle ] of String = ( 'bsSolid', 'bsClear', 'bsHorizontal', 'bsVertical',
    'bsFDiagonal', 'bsBDiagonal', 'bsCross', 'bsDiagCross' {$ifdef FPC}, '', '' {$endif});
var RsrcName: String;
    Updated: Boolean;
    KF: TKOLForm;
    i: Integer;
